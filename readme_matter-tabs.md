...
MatterStepper es un web component que genera un componente de tabs. Forma parte del catálogo corporativo de Web Components de la Junta de Andalucía.
...
1. Importar el compilado del web-component en el proyecto actual:
    ```bash
    import from '@matter/matter-tabs/dist/matter-tabs'
    ```
2. Una vez importado, llamar a dicho web-component a partir de su etiqueta propia individual. Esta llamada se debe realizar desde el fichero .html donde se quiera mostrar el web-component:
    ...
    tabs='[
    {"label": "Inicio", "content": "Bienvenido a la página de inicio",  "disabled": false},
    {"label": "Servicios", "content": "Explora los servicios que ofrecemos", "disabled": false},
    {"label": "Contacto", "content": "Ponte en contacto con nosotros", "disabled": false},
    {"label": "FAQs", "content": "Respuestas a preguntas frecuentes", "disabled": true}
  ]'
    currentTab='0'>
  </matter-tabs>
    ```
...
| `tabs`           | `Array`  | Lista de pestañas en formato `[{ label: "Inicio", content: "Texto", disabled: false }]`. |
| `currentTab`     | `Number` | Índice de la pestaña activa (basado en la posición dentro del array `tabs`). |
| `tabsUrl`        | `String` | URL opcional para cargar dinámicamente las pestañas desde un JSON externo. |
  ...
  plugins: [
    ...
    new CopyPlugin({
      patterns: [
        ...
        { from: "./demo/matter-tabs.stories.js", to: 'story' + '/' + PackageType + '/' + PackageName },
      ],
    }),  ],
